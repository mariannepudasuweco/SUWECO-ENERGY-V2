        ...
        let boqBudgets = {}; // { projectId: { 'A1': 1000, ... } }
                ...
                <div style="display: grid; grid-template-columns: repeat(auto-fit, minmax(200px, 1fr)); gap: 16px; margin-bottom: 24px;">
                    <div class="card" style="background: #eff6ff; border: 1px solid #bfdbfe; display: flex; align-items: center; gap: 16px; padding: 16px;">
                        ...
                        <div>
                            ...
                            <div style="font-size: 1.25rem; font-weight: 700; color: #1e3a8a;">${formatCurrency(grandBudget)}</div>
                        </div>
                    </div>
                    <div class="card" style="background: #f0fdf4; border: 1px solid #bbf7d0; display: flex; align-items: center; gap: 16px; padding: 16px;">
                    ...
                    <div class="card" style="background: #fef2f2; border: 1px solid #fecaca; display: flex; align-items: center; gap: 16px; padding: 16px;">
                    ...
                    <div class="card" style="background: #f8fafc; border: 1px solid #e2e8f0; display: flex; align-items: center; gap: 16px; padding: 16px;">
                        <div style="width: 40px; height: 40px; border-radius: 8px; background: #64748b; color: white; display: flex; align-items: center; justify-content: center;">
                            <svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><circle cx="12" cy="12" r="10"/><polyline points="12 6 12 12 16 14"/></svg>
                        </div>
                        <div>
                            <div style="font-size: 0.85rem; color: #334155; font-weight: 600; margin-bottom: 4px;">Grand Total</div>
                            <div style="font-size: 1.25rem; font-weight: 700; color: #0f172a;">${formatCurrency(grandTotal)}</div>
                        </div>
                    </div>
            ...
            const renderRow = (item, isSub = false) => {
                ...
                if (item.isParent) {
                    ...
                    if (item.expanded) {
                        ...
                        if (!hasVisibleSub && !boqSearchQuery) {
                            // If no subitems match the tab filter, hide the parent too
                        }
                    }
                    ...
                    return `
                        <tr style="background: var(--bg-surface); border-bottom: 1px solid var(--border-color); cursor: pointer;" onclick="toggleBoqParent('${item.code}')">
                            ...
                            <td style="padding: 12px 16px; text-align: right; color: #ef4444; font-weight: 500;">${formatCurrency(item.unpaid)}</td>
                ...
                return `
                    <tr style="border-bottom: 1px solid var(--border-color); ${isSub ? 'background: var(--bg-body);' : ''}">
                        <td style="padding: 12px 16px; color: #3b82f6; font-weight: 500; ${isSub ? 'padding-left: 40px;' : ''}">${item.code}</td>
                        <td style="padding: 12px 16px; font-size: 0.85rem;">${item.name}</td>
                        <td style="padding: 12px 16px; text-align: right; color: #3b82f6; font-weight: 600;">${formatCurrency(item.budget)}</td>
                        <td style="padding: 12px 16px; text-align: right; color: #22c55e; font-weight: 500;">${formatCurrency(item.paid)}</td>
                        <td style="padding: 12px 16px; text-align: right; color: #ef4444; font-weight: 500;">${formatCurrency(item.unpaid)}</td>
                        <td style="padding: 12px 16px; text-align: right; font-weight: 600;">${formatCurrency(item.total)}</td>
