...
const scriptMatch = html.match(/window\.renderBoqChargingView = function\(\) \{([\s\S]*?)\};/);
let code = scriptMatch[1];

// Extract the renderRow function and the tree loop
const testCode = `
...
fs.writeFileSync('test_render_logic.js', testCode);
